...
确定本轮编译次数 attempt_no：
- 若 .build-logs/F-{feature-id}/attempt-*.log 不存在，则 attempt_no = 1
- 若已存在，则取最大编号 + 1
- 本次 hm_build 步骤最多允许自动修复 5 次；因此 attempt_no 最大可到 6（第 6 次只用于验证第 5 次修复）
- 若计算出的 attempt_no > 6，按进度日志写入规则将 hm_build[unstart] 改为 hm_build[need_human_info] 修复 5 次后仍未编译通过，并追加 human_info[unstart]，然后回到主循环顶部

---

**Phase 2：主 Agent 执行一次编译**

执行编译命令，并将日志写入 TOOL_ROOT 下的 .build-logs：

(cd {HM_ROOT} && hvigorw assembleHap --mode module -p product=default --analyze=normal --parallel --incremental --daemon 2>&1) > .build-logs/F-{feature-id}/attempt-{attempt_no}.log

编译命令执行后，主 Agent 只检查日志文件是否存在且非空：
- 若日志文件存在且非空 → 进入 Phase 3
- 若日志文件不存在或为空 → 不再召唤 subagent，按进度日志写入规则将 hm_build[unstart] 改为 hm_build[need_human_info] 编译命令未生成日志，并追加 human_info[unstart]，然后回到主循环顶部

主 Agent 不解析日志中的 BUILD SUCCESS、ERROR、BUILD FAILED，也不根据日志判断编译是否成功。

---

**Phase 3：召唤 hm-build-fixer 判断日志并修复一次**

召唤 hm-build-fixer 执行任务：
任务：处理功能点 {feature-id} 的第 {attempt_no} 次编译日志
参数：
...
**Phase 4：读取 subagent 结果并决定是否继续循环**
...
status = fixed →
  1. 清空 .migration/hm-dev-result.md
  2. attempt_no = attempt_no + 1
  3. 若 attempt_no <= 6：回到 Phase 2 重新编译
  4. 若 attempt_no > 6：按进度日志写入规则将 hm_build[unstart] 改为 hm_build[need_human_info] 修复 5 次后仍未编译通过，并追加 human_info[unstart]，然后回到主循环顶部
